...
# --- 1. GESTÃO DE ESTADO E SEGURANÇA (GATEKEEPER) ---
...
def realizar_login():
    ...
    for nivel, cred in config.CREDENCIAIS.items():
        if usr == cred["user"] and pwd == cred["pass"]:
            ...
            st.rerun() # Força a limpeza da tela e recarrega o sistema validado
            return
            
    st.error("Acesso Negado. Credenciais inválidas.")

# Bloqueio Absoluto: Nada abaixo desta linha renderiza sem login
...
# --- 2. NAVEGAÇÃO LATERAL (MENU E LOGOFF) ---
with st.sidebar:
    st.title("📟 Menu")
