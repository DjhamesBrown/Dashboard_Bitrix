...
def renderizar_aba_gestao():
    ...
    if not df_charts.empty:
        ...
        with col_v1:
            ...
            if opcao_visao_vol == "Gráfico (Top 15)":
                ...
                evento_tabela_vol = st.dataframe(df_vol_cli, use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", key="tabela_vol_cli_todos", height=350)
        ...
        with col_v2:
            ...
            fig_v2 = px.bar(df_vol_ana, y='Responsável', x='Total de Chamados', orientation='h', title="Distribuição de Carga de Tickets (por Analista)", text_auto='.0f', color='Responsável')
    ...
    if not df_charts.empty:
        ...
        with col_t1:
            if "Esforco_Tarefas_h" in df_charts:
                ...
                if opcao_visao_cli == "Gráfico (Top 15)":
                    ...
                    evento_tabela = st.dataframe(df_show_tabela, use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", key="tabela_cli_horas_todos", height=350)
        ...
        with col_t2:
            if "Esforco_Tarefas_h" in df_charts:
                ...
                fig_t2 = px.bar(df_tempo_ana, y='Responsável', x='Esforco_Tarefas_h', orientation='h', title="Alocação de Tempo por Analista", text='Tempo Texto', color='Responsável')
    ...
    if not df_charts.empty:
        ...
        if not df_reabertos.empty:
            ...
            st.markdown("**Matriz de Justificativas (Tickets Multi-Dias / Reabertos)**")
            st.dataframe(df_show_reabertos, use_container_width=True, hide_index=True)
        else:
            st.success("✅ Excelente! Nenhum chamado registrou retrabalho ou reabertura multi-dias no período selecionado.")
    ...
    with st.expander("📄 Visualizar Matriz Bruta (Auditoria)"):
        if not df_charts.empty:
            cols_view = ["ID", "Responsável", "Cliente", "Fase Nome", "Lead_Time_Bruto", "Esforco_Formatado", "Motivo Abertura", "Motivo Fechamento", "Data Formatada", "Último Follow-up"]
